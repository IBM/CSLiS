...
mblk_t *lis_lock_contention_msg(void)
{
    ...
    sprintf((char *)(mp->b_wptr), "Total Lock Contention: %u\n", 
		tot_contention) ;
	...
	sprintf((char *)(mp->b_wptr), "%10u(%d%%): %s last-owner: %s %d\n",
		(*p)->contention_cnt,
...
mblk_t *lis_sem_contention_msg(void)
{
    ...
    sprintf((char *)(mp->b_wptr), "Total Semaphore Contention: %u\n", 
	tot_contention) ;
    while (*mp->b_wptr) mp->b_wptr++;
	...
	sprintf((char *)(mp->b_wptr), "%10u(%d%%): last-owner: %s %d\n",
		(*p)->contention_cnt,
		tot_contention ?  ((*p)->contention_cnt * 100) / tot_contention
			       : 0,
		(*p)->owner_file ? (*p)->owner_file : "Unknown",

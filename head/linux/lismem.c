...
#ident "@(#) CSLiS lismem.c 7.11 2022-10-26 15:30:00 "
...
void *lis__kmalloc(int nbytes, int class, int use_cache)
{
    ...
    if (!use_cache || nbytes > SIZE_LIS_MAX)
    {
	p = kmalloc(nbytes, class) ;
  if (p == NULL)
  {
    printk("lis__kmalloc kmalloc failed size %d\n", nbytes);
  }
    }
    else
  ...
  {
    printk("lis__kmalloc slab not found size %d\n", nbytes);
  return(NULL) ;
  }
  ...
  if (p == NULL)
  {
    printk("lis__kmalloc kmem_cache_alloc failed size %d\n", nbytes);
  }

...
#ident "@(#) CSLiS queue.c 7.111 2024-05-07 15:30:00 "
...
mblk_t *lis_queue_contention_msg(void)
{
    ...
    sprintf((char *)(mp->b_wptr), "Total Queue Contention: %u\n", 
	tot_contention) ;
	...
	sprintf((char *)(mp->b_wptr), "%10lu(%ld%%): %s last-owner: %s %d\n",
		(*p)->q_contention_cnt,
...
int	lis_check_q_magic(queue_t *q, char *file, int line)
{
	...
	printk("%s #%u: Queue pointer is NULL\n", file, line) ;
       	...
       	{
	printk("%s #%u: q=%p Queue magic number is 0x%lx, should be 0x%lx\n",
       		file, line, q, q->q_magic, Q_MAGIC) ;
       	}
    return(0) ;
...
lis_allocq( const char *name )
{
  ...
  if (LIS_DEBUG_ADDRS)
  {
     printk("lis_alloc: q=%p magic=0x%lx \n", q, q->q_magic) ;
     printk("lis_alloc: q_other=%p magic=0x%lx \n", q->q_other, q->q_other->q_magic) ;
  }
...
lis_freeq( queue_t *q )
{
	...
	if (!LIS_CHECK_Q_MAGIC(q)) continue ;
        ...
        if (LIS_DEBUG_ADDRS)
        {
           printk("lis_freeq: q=%p  magic=0x%lx \n", q, q->q_magic) ;
	}

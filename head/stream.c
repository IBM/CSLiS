 ...
 * RCS Id          : $Id: stream.c,v 1.3 2006/06/19 15:56:10 steve Exp $
...
//	if (!LIS_CHECK_Q_MAGIC(q))
        if ((q) && ((q)->q_magic != Q_MAGIC)) 
	{
           if ( LIS_DEBUG_CLOSE )
              printk("queuerun Contention: queue flags are 0x%lx\n",q->q_flag) ;
           if (q->q_flag & (QCLOSEWT | QWANTENAB))    // When QCLOSEWT or QWANTENAB set, do not schedule

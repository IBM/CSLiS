 ...
 * RCS Id          : $Id: buffcall.c,v 1.5 2006/06/19 15:56:10 steve Exp $
...
#ident "@(#) CSLiS buffcall.c 7.11 2022-10-26 15:30:00 "
...
struct lis_timer_list    lis_bctl ;     /* timer structure for bufcall */
#else
...
{
#else
static void bc_timeout(struct timer_list *tmout_tl)
{
...
#else
        lis_tmout(&lis_bctl, (tmout_fcn_t *)bc_timeout, 0, BUFCALL_N_TICKS) ;
#endif
    }
    else
	lis_spin_unlock_irqrestore(&lis_bc_lock, &psw) ;
 ...
 *
 * You must lis_spin_lock_irqsave(&lis_bc_lock, &psw) prior to calling 
 * this routine.
 */

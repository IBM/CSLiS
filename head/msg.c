 ...
 * RCS Id          : $Id: msg.c,v 1.2 2006/05/18 17:06:05 steve Exp $
...
#else
static struct mdbblock * allochdr(unsigned int priority)
#endif
{
#if !defined(CONFIG_DEV)
...
#if defined(CONFIG_DEV)
static char *
allocdb(int size, unsigned int priority, char *file_name, int line_nr, int flags)
#else
static char *
allocdb(int size, unsigned int priority, int flags)
#endif
{
#if !defined(CONFIG_DEV)
    static char *file_name = "__FILE__" ;
    ...
    {
      rtn = (char *)lis_malloc(size, GFP_KERNEL|flags, 1, file_name, line_nr);
    }
    else   
    {
...
{
#if defined(CONFIG_DEV)
    struct mdbblock *rtn = allochdr(priority, file_name, line_nr);
#else
    struct mdbblock *rtn = allochdr(priority);
#endif
    char *buff;
    (void)priority;
    if (rtn == NULL)
...
#if defined(CONFIG_DEV)
	if ((buff = allocdb(size, priority, file_name, line_nr, 0)) == NULL)
#else
	if ((buff = allocdb(size, priority, 0)) == NULL)
#endif
	{
...
#if defined(CONFIG_DEV)
    if (base && freeinfo && (rtn  = allochdr(priority, file_name, line_nr)) != NULL)
#else
    if (base && freeinfo && (rtn  = allochdr(priority)) != NULL)
#endif
    	initb(rtn, (char *)base, size, freeinfo);
...
lis_freedb(mblk_t *bp, int free_hdr)
{
    	...
    	if (dp)
	{
		...
		if (dp->db_size > FASTBUF)
		    ...
		    db_base = (char *)(dp->db_base) ;	/* area to free */

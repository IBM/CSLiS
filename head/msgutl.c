 ...
 * RCS Id          : $Id: msgutl.c,v 1.1.1.1 2005/04/12 20:27:05 ragnar Exp $
...
lis_pullupmsg(mblk_t *mp, int length)
{
     ...
     * for its buffer.  Setting nbytes to FASTBUF+1 ensures this.
     */
    nbytes = length ;
    if (nbytes <= FASTBUF) nbytes = FASTBUF+1 ;
...
mblk_t	* _RP lis_msgpullup(mblk_t *mp, int len)
{
    ...
    if (len < 0)			/* pull up all bytes */
    {					/* count bytes in orginal msg */
	for (len = 0, bp = mp;
	     bp != NULL && bp->b_datap->db_type == msg_type;
	     bp = bp->b_cont 
	    )
	{
    ...
    for (nbytes = 0;
	 len > 0 && mp != NULL && mp->b_datap->db_type == msg_type;
	...
	if ((nbytes = mp->b_wptr - mp->b_rptr) > 0)
	{
	    ...
	    if (cpbytes == len && cpbytes < nbytes)
	    {					/* mp has leftover bytes */

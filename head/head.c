...
#if (defined(__s390x__) ) && (LINUX_VERSION_CODE > KERNEL_VERSION(4, 12, 0))
...
copyin_msgpart( struct file *f, stdata_t *hd, mblk_t **m, char *sb, int wroff )
{
    ...
    */
    ...
    if ( (*m=(mblk_t*)allocb(kbuf.len+wroff,BPRI_RETRY))==NULL)
	return(-ENOMEM);
...
lis_qdetach(queue_t *q, int do_close, int flag, cred_t *creds)
{
     ...
     * they won't be called.  This must be done before we check the QRUNNING
     * flag.  See the note on a race condition in queuerun().
     */
...
{
    mblk_t *mread = allocb(sizeof(ulen),BPRI_RETRY);
    unsigned *uptr ;
...
{
    mblk_t *mf = allocb(2*sizeof(int),BPRI_RETRY); /*sizeof mflush msg (?)*/
    if (mf == NULL)
	return(-ENOMEM);
    else {
...
/*  -------------------------------------------------------------------  */
/* lis_process_rput - called from stream head read service to actually 
 * process msg
 *
...
/*****************************************************/
    ...
    if ( LIS_DEBUG_IOCTL )
	printk("lis_wait_for_wiocing: after SET_SD_FLAG\n");
    if (tmout != INFTIM)
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,14,0)
	lis_tmout(&lis_tl,lis_do_tmout,(long)hd, SECS_TO(tmout));
#else
    ...
    if ( LIS_DEBUG_IOCTL )
	printk("lis_wait_for_wiocing: return(0)\n");
    return(0);			/* got msg */
...
#define RTN(v)	do { errv=(v); goto return_point; } while (0)
  ...
  again:				/* wioc lock is always held when here */
    ...
    if ( LIS_DEBUG_IOCTL )
	printk("lis_strdoioctl: call lis_wait_for_wiocing\n");
    err = lis_wait_for_wiocing(hd,ioc->ic_timout, ignore_errors) ;
    if ( LIS_DEBUG_IOCTL )
	printk("lis_strdoioctl: after lis_wait_for_wiocing\n");
    ...
    case M_COPYIN:
	{
		...
		|| (dat=allocb(len,BPRI_RETRY))==NULL
...
#else  /*  below release level for GA RHEL 6.1 */
                {
		    cr->cp_rval=(caddr_t)(-err);
                }
#endif
...
#define RTN(e)  do { err=(e); goto return_point; } while (0)
	...
	if (LIS_DEBUG_WRITE)
	  ...
	  printk(
	    "strwrite: stream %s: write length %lu less than min size of %lu\n",
			hd->sd_name,
			ulen, hd->sd_minpsz) ;
#else
	  printk(
...
#if (defined(_S390X_LIS_) || defined(_PPC64_LIS_) || defined(_X86_64_LIS_))
	      printk(
	      "strread: stream %s: RDOPT=0x%x user buffer %lu bytes, "
	      "read %d error %d\n",
...
#define RTN(v)	do { err=(v); goto return_point; } while (0)
	...
	printk("lis_strioctl(i@0x%p/%d,f@0x%p/%d,cmd=0x%x,arg=0x%lx)"
        ...
        printk("lis_unlock_ioctl(i@0x%p/%d,f@0x%p/%li,cmd=0x%x,arg=0x%lx)"
	       ...
	       i, (i?I_COUNT(i):0),
	       f, (f?F_COUNT(f):0),
	       cmd, arg, (i?getmajor(GET_I_RDEV(i)):0)) ;  
//  	       cmd, arg,1) ;
    ...
    case I_LIS_PUTMSG:
    	{
	    putpmsg_args_t	putargs ;
	    if (!(err = lis_check_umem(f, VERIFY_READ,
				(char *)arg, sizeof(putargs))) &&
	        !(err = lis_copyin(f,&putargs,(char *)arg,sizeof(putargs))))
	    {

	        err = lis_strputpmsg(i, f, putargs.ctl, putargs.dat,
				 putargs.band, putargs.flags);
    ...
    case I_LIS_GETMSG:
    	{
	     getpmsg_args_t	getargs ;
	     if (!(err = lis_check_umem(f, VERIFY_READ,
				(char *)arg, sizeof(getargs))) &&
	         !(err = lis_copyin(f,&getargs,(char *)arg,sizeof(getargs))))
	     {
	         err = lis_strgetpmsg(i, f, getargs.ctl, getargs.dat,
				 getargs.bandp, getargs.flagsp, 1);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(4,14,0)
                lis_tmout( &lis_tl, (tmout_fcn_t *) lis_do_rd_tmout,
...
//		       " \"%s\" >> NULL queue ptrs\n", hd->sd_name) ;
    ...
    case I_LIS_FDETACH:
        ...
        RTN(lis_ioc_fattach( f, (struct filename *) arg ));

    case I_LIS_FDETACH:
	if (LIS_DEBUG_IOCTL)
	    printk( "lis_strioctl(...,I_LIS_FDETACH,\"%s\")\n",
		    ((struct filename *)arg)->name );
	RTN(lis_ioc_fdetach( (struct filename *)arg ));
#endif
    /**/
	...
	stats->num_cpus = lis_num_cpus;
        if (stats->num_cpus > LIS_NR_CPUS)
        {
            stats->num_cpus = LIS_NR_CPUS;
        }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,10,0)
   /*  For Kernel 3.10 and greater... mark READ Mode off since closing file for good */

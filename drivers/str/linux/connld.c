...
#ident "@(#) CSLiS connld.c 7.11 2022-10-26 15:30:00 "
			    ...
			    cred_t *credp )
{
	...
	cmn_err( CE_CONT,
		 "connld_open(): bad reference to stream head\n" );
	return -EINVAL;
    }
    if (!F_ISSET(head->sd_flag, STFIFO)) {
	cmn_err( CE_CONT,
		 "connld_open(): not even a FIFO!!\n" );
	return -EPIPE;
    }
    if (!head->sd_peer || head->sd_peer == head) {
	cmn_err( CE_CONT,
		 "connld_open(): this STREAM just is a FIFO, not a pipe\n" );
	return -EPIPE;
    }
	...
	if (head != FILE_STR(oldf)) {
	    cmn_err( CE_CONT,
		     "connld_open(): invalid sd_file 0x%p\n", head->sd_file );
	    return -EINVAL;
	}
	...
	if ((error = lis_sendfd( head, -1, f1 )) < 0) {
	    cmn_err( CE_CONT,
		     "connld_open(): sendfd(h@0x%p,-1,f@0x%p) failed - %d\n",
		     head, f1, error );
	 ...
	 */	
	{
	    ...
	    *devp = GET_I_RDEV(FILE_INODE(f0));
	    ...
	    cmn_err( CE_CONT,
		     ...
		     oldhead,
		     LIS_SD_REFCNT(oldhead), LIS_SD_OPENCNT(oldhead),
		     head,
	...
	if (F_ISSET(head->sd_flag,STCONNLD)) {
	    cmn_err( CE_CONT,
		     "connld_open(): previously pushed 'connld'\n" );
	    return -EINVAL;
	}
